...
const db = __importStar(require("./db")); // Import our initialized DB functions
        ...
        app.post('/api/orders', async (req, res) => {
            ...
            try {
                ...
                for (const item of orderItemsToInsert) {
                    await db.run('INSERT INTO order_items (order_id, product_id, name, price, quantity) VALUES (?, ?, ?, ?, ?)', [orderId, item.product_id, item.name, item.price, item.quantity]);
                }
                // 3. Update Stock Levels and Finalize
                for (const productId of productIdsUsed) {
                    await db.run('UPDATE products SET stock = stock - ? WHERE id = ? AND stock >= ?', [cartItems.find((c) => c.productId == productId)?.quantity || 0, productId, cartItems.find((c) => c.productId == productId)?.quantity || 0]);
                }
