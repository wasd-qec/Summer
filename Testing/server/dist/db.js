...
exports.query = exports.get = exports.run = exports.seedProducts = exports.initializeDatabase = void 0;
...
async function initializeDatabase() {
    if (db)
        return db; // Avoid re-initializing if already done
    try {
...
async function seedProducts() {
    ...
    for (const product of products) {
        await db.run(`INSERT OR IGNORE INTO products (id, name, description, price, stock, image_url) 
                      VALUES ((SELECT id FROM products WHERE name = ?), ?, ?, ?, ?, ?)`, [product.name, product.name, product.description, product.price, product.stock, product.image_url]);
    }
